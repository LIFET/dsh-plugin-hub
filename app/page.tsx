...
import { selectFeaturedPlugins, selectRecentPlugins } from "@/lib/plugin-screening.mjs";
...
export default async function Home() {
  ...
  const newcomers = selectRecentPlugins(registry.plugins, 6)
    .filter((plugin) => !featured.some((item) => item.id === plugin.id));
  const data: PluginRegistryData = { ...registry, plugins: [...featured, ...newcomers] };
