...
export function classifyInspectionFailure(value) {
  ...
  return /response (?:exceeded|too large)|unsafe repository path|invalid github repository|invalid json/iu.test(message)
    ? "uninspectable"
    : "transient";
}
...
export function sanitizeRegistryInstallEvidence(registry) {
  ...
  return {
    ...
    plugins: registry.plugins.map((plugin) => {
      ...
      const allowed = plugin.manifest?.state === "verified" && screeningState === "clear";
