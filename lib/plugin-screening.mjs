...
export function normalizeOwnerParam(value) {
  const owner = String(value || "").trim();
  if (!/^[A-Za-z0-9](?:[A-Za-z0-9]|-(?=[A-Za-z0-9])){0,38}$/u.test(owner)) return "";
  return owner;
}

export function selectRecentPlugins(plugins, limit = 6) {
  return [...(Array.isArray(plugins) ? plugins : [])]
    .filter((plugin) => plugin?.screening?.state !== "blocked")
    .sort((a, b) => {
      const tb = Date.parse(b?.added || b?.discovery?.firstSeenAt || 0);
      const ta = Date.parse(a?.added || a?.discovery?.firstSeenAt || 0);
      return (Number.isFinite(tb) ? tb : 0) - (Number.isFinite(ta) ? ta : 0) || comparePluginsByEvidence(a, b);
    })
    .slice(0, limit);
}
...
export function normalizeRepositoryUrl(value) {
  ...
  const match = raw.match(/^https:\/\/github\.com\/([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+?)(?:\.git)?\/?$/u);
  if (!match) return "";
...
export function catalogPageTitle({
  ...
  if (q) return lang === "en" ? `Search ${q}` : `搜索 ${q}`;
  const normalizedOwner = normalizeOwnerParam(owner);
  if (normalizedOwner) return normalizedOwner;
  if (category && category !== "all") return categories?.[category]?.[lang] || fallback;
  return fallback;
}
